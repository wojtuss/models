...
def train(args):
    ...
    def train_one_batch(data):
        ...
        print("varnames in fetch_vars are")
        print(var_names)
        if args.parallel:
    ...
    def test(iter_num):
        ...
        for data in test_reader():
            ...
            __, test_seq_error = error_evaluator.eval(exe)
            print("\nTime: %s; Iter[%d]; Test seq error: %s.\n" % (
                time.time(), iter_num, str(test_seq_error[0])))
            print(test_seq_error)

            fluid.io.save_inference_model(args.save_model_dir, ["pixel","label"], fetch_vars, exe, main_program=inference_program)
            print("Model saved!")
