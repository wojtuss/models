...
add_arg('model_save_dir',   str,   'my_saved_model',     "A path to save model.")
...
def build_program(main_prog, startup_prog, args, data_args):
    ...
    return py_reader, map, nmsed_out, image, gt_box, gt_label, difficult
...
def eval(args, data_args, test_list, batch_size, model_dir=None):
    ...
    test_py_reader, map_eval, nmsed_out, image, gt_box, gt_label, difficult = build_program(
        main_prog=test_prog,
    ...
    fluid.io.save_inference_model(
        args.model_save_dir,
        [image.name, gt_box.name, gt_label.name, difficult.name],
        [nmsed_out, cur_map, accum_map],
        #  [image.name],
        #  [nmsed_out],
        exe,
        test_prog)
...
if __name__ == '__main__':
    ...
    if not os.path.exists(args.model_dir):
        raise ValueError(
            "The model path [%s] does not exist." % (args.model_dir))
